...
**CLI-Tool `lrns`** mit Subcommands: `status`, `path`, `identity`, `probe`, `interfaces`, `connect`, `selftest`. Davon voll implementiert: `identity`, `connect`, `selftest` (Zwei-Adress-Modus für Multi-Daemon-Topologien, 7 Modi: `all`, `link`, `packet`, `ratchet-basic`, `ratchet-enforced`, `bulk-transfer`, `ratchet-rotation`). Die anderen sind Gerüste.
