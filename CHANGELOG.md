...
### Added
- **LoRa test coverage gaps 1-4** — 5 new LoRa integration tests covering previously untested scenarios: file size sweep (64B–50KB, 6 sizes), higher frame loss (4-drop and 6-drop proxy tests on 10KB files), link establishment under loss (drop 2 link-sized frames then verify transfer), and sequential bidirectional transfer (a→b then b→a on same LoRa link). New TOML files: `lora_lrncp_size_sweep.toml`, `lora_lrncp_proxy_4drop.toml`, `lora_lrncp_proxy_6drop.toml`, `lora_lrncp_link_loss.toml`, `lora_lrncp_bidir.toml`.
- **Proxy rule max_size filter** — proxy rules now support `max_size=N` (only match frames with payload <= N bytes, 0 = no limit). Combined with existing `min_size`, this enables targeting specific packet types by size range (e.g., link requests at 50-130 bytes vs announces at ~167 bytes vs data segments at ~490 bytes).
- **LoRa implementation comparison matrix (16 tests)** — comprehensive LoRa integration test suite covering all implementation pairings: Rust↔Rust (5 tests), Python↔Python (5 tests), and cross-implementation (6 tests). Tests cover push, fetch, auth, proxy (with frame dropping), and LoRa-to-TCP bridge scenarios. Cross-impl tests mix `lrncp`/`rncp` tools and `lrnsd`/`rnsd` daemons over real RNode hardware. New TOML files: `lora_rncp_{push,fetch,auth,proxy,bridge}.toml` (Python↔Python), `lora_lrncp_push_to_python.toml`, `lora_rncp_push_to_rust.toml`, `lora_lrncp_fetch_from_python.toml`, `lora_rncp_fetch_from_rust.toml`, `lora_lrncp_auth_to_python.toml`, `lora_lrncp_bridge_python_relay.toml` (cross-impl).
- **File transfer test framework: fetch, auth, and negative testing** — `file_transfer` integration test step now supports `mode` (push/fetch), `receiver_flags`, `sender_flags`, `auth_from` (identity-based link authentication), `expect_result` (success/failure for negative tests), and `fetch_path` (override remote path for jail violation tests). New `parse_identity_hash_from_print_identity()` handles both lrncp and rncp `-p` output formats. Six new Docker-based integration tests: `lrncp_fetch`, `lrncp_fetch_cross`, `lrncp_fetch_auth`, `lrncp_fetch_jail`, `lrncp_auth`, `lrncp_auth_reject`.
- **Proxy rule enhancements: min_size and skip** — proxy rules now support `min_size` (only match frames with payload >= N bytes) and `skip` (forward N matching frames before activating). Enables precise targeting of specific packet types (e.g., dropping only large resource data segments while ignoring announces and link requests).

### Fixed
- **Resource retransmit timing matched to Python** — receiver-side (`IncomingResource`) and sender-side (`OutgoingResource`) timeout formulas were too conservative for LoRa, causing 40% of transfers to take 3x longer (retransmit timeout 5-8s per lost packet vs Python's 3s). Fix: adaptive timeout factor (4 initially, reduced to 2 after first data received, matching Python `PART_TIMEOUT_FACTOR_AFTER_RTT`), per-retry progressive backoff (+500ms per retry, Python `PER_RETRY_DELAY`), retry grace time (250ms, Python `RETRY_GRACE_TIME`), sender grace time (10s, Python `SENDER_GRACE_TIME`), and proof timeout factor (3x RTT + 10s grace, Python `PROOF_TIMEOUT_FACTOR`). Seven new constants in `resource/mod.rs` matching Python `Resource.py:126-134`. Result: Rust push 1KB over LoRa improved from 10.2-32.5s (22s spread) to 7.8-7.9s (0.03s spread), now faster than Python (9.2s). Proxy with 2 drops: 25.5s → 14.5s.
- **Receiver retransmit REQ rebuilt with only missing parts** — on timeout, `IncomingResource::poll()` retransmitted the stale `last_req` containing ALL originally-requested part hashes, including parts already received. The sender wasted airtime resending data the receiver already had. Fix: call `build_request()` on timeout to produce a fresh REQ with only the currently-missing parts (matching Python `Resource.py:622` `request_next()`). Also cap `per_part_tof` at `rtt_ms` to prevent contaminated EIFR measurements (from dropped frames inflating req-to-first-part elapsed time) from bloating timeouts. Result: proxy with 2 drops at slow profile improved from 31.7s to 16.6s (Rust), now 31% faster than Python (24.0s). Medium profile outlier rate reduced from 20% to 0%.
- **Fetch listener crash in integration tests** — in fetch mode, `/tmp/received` was created only on the sender node but the listener on the receiver node exits if `-s /tmp/received` directory doesn't exist (Python rncp.py:103-106). The listener crashed silently before registering its destination, making all LoRa fetch tests fail. Fix: create `/tmp/received` on the receiver node unconditionally.
- **LoRa announce wait time for file transfer tests** — increased from 5s to 30s for tests with RNode interfaces. On LoRa at 2.73 kbps, announce propagation (airtime + jitter) can exceed 5 seconds.
- **Resource retransmit timeout not resetting** — `OutgoingResource::poll()` in `Transferring` state did not update `last_activity_ms` when incrementing retries, causing all retries to fire immediately instead of being spaced by the timeout interval. `AwaitingProof` state now also retries instead of immediately failing on first timeout, allowing the receiver time to complete and send the proof.
- **Shared-instance resource retransmissions blocked by packet dedup** — resource retransmissions produce identical raw bytes (no nonce in `build_raw_data_packet`), so the packet hash matched the original. When routed through a shared instance daemon, `process_incoming()` rejected them as duplicates. Fix: skip dedup for packets from a local client destined for a known link (matching Python's behavior where outbound traffic never passes through `process_incoming`). Also skip hash caching for local-client link traffic to avoid blocking future retransmits.
