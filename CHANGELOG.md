...
### Added
- **Docker-based integration test framework (`reticulum-integ`)** — TOML-defined test scenarios spin up mixed Rust/Python node topologies in Docker containers with automatic config generation, TCP port assignment, identity creation, readiness polling, and step-by-step assertions. 10 test scenarios covering chains, triangles, rings, restarts, and mixed Rust/Python topologies. Implemented step types: `wait_for_path`, `rnprobe`, `sleep`, `exec`, `restart`. Known failing test: `five_node_mesh` — 5-node ring topology does not converge to shortest paths (3 hops instead of expected 2), indicating announce replacement does not work for competing multi-hop paths.
