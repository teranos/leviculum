...
### Added
- **Docker-based integration test framework (`reticulum-integ`)** — TOML-defined test scenarios spin up mixed Rust/Python node topologies in Docker containers with automatic config generation, TCP port assignment, identity creation, readiness polling, and step-by-step assertions. Three test scenarios: `basic_probe` (2 nodes, direct 1-hop probe), `probe_through_relay` (3-node chain, 2-hop probe through Python relay), `path_self_healing` (triangle topology regression test verifying direct 1-hop paths win over 2-hop relay paths). Framework generates docker-compose.yml, Reticulum INI configs, and transport identities from TOML topology definitions. Step types: `wait_for_path`, `rnprobe`, `sleep`, `rnpath`, `rnstatus`, `exec`, `restart`.
