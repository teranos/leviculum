...
### Added
- **Shared instance registration delay (Block B)** — when a local IPC client announces a destination for the first time, the daemon delays network rebroadcast by 250ms to batch multiple registrations during startup. Repeat announces from the same client are rebroadcast immediately. Tracks per-client destination maps and a persistent `local_client_known_dests` set that survives client disconnect.
