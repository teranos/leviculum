...
- **Shared instance interop tests** — 2 Python interop tests validating Unix socket IPC: `test_shared_instance_receive_announce` (Python announces → Rust receives via Unix socket) and `test_shared_instance_send_announce` (Rust sends announce → Python creates path). Uses `--share-instance`/`--instance-name` flags added to `scripts/test_daemon.py`.
